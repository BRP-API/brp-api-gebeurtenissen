...
BeforeStep(async function (this: ICustomWorld, { pickleStep }) {
  ...
  switch (pickleStep.type) {
    ...
    case 'Outcome':
      ...
      break;
    default:
      if (pickleStep.text.startsWith('het adres') || pickleStep.text.startsWith('de persoon') || pickleStep.text.startsWith('de aangifte')) {
        await createHuidigAanduiding.call(this);
      }
  }
});
...
function copyIdIfExpectedIsExternalEventAndResultHasId(expected: any, result: any) {
  if (expected instanceof Event && !expected.intern && result && result.id) {
