...
export async function raadpleegGebeurtenissenVoorAbonnee(afnemer: Afnemer, abonneeNaam: string, limit?: bigint, persoon?: Persoon, cursor?: string): Promise<any> {
    ...
    const response = await fetch(`${process.env.GEBEURTENISSEN_BASE_URL}/api/brp/abonnees/${abonneeNaam}/gebeurtenissen${uriParamsString}`, {
        method: 'GET',
        headers: {
    ...
    logger.debug(`raadpleegGebeurtenissenVoorAbonnee afnemer: '${afnemer?.aanduiding}', abonneeNaam: ${abonneeNaam}`, { response: response });
    logger.info(`GET /api/brp/abonnees/${abonneeNaam}/gebeurtenissen${uriParamsString} >>> status: ${response.status}`);

    return await response.json();
};

export async function publiceerGebeurtenis(gebeurtenistype: string, persoon: Persoon) {
    let data: any = {
        c01: {
            e0110: `${persoon.a_nr}`
        }
    ...
    let requestBody: any = {
        ...
        metadata: [
            {
                naam: "activiteitId",
                waarde: `publiceerGebeurtenis-${persoon.geslachts_naam}-` + Math.floor(Math.random() * 10000).toString()
            }
        ]
    }
    ...
    logger.debug(`publiceerGebeurtenis afnemer: type: ${gebeurtenistype}, requestBody: ${requestBody}`, { response: response });
    logger.info(`POST /personen/gebeurtenissen >>> status: ${response.status}`, requestBody);
}
