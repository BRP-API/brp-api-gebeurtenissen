...
export function createLo3PlInsertStatement(persoon: Persoon): SqlStatement {
  ...
  return new SqlStatement(`INSERT INTO public.lo3_pl(${insertPart}) VALUES(${valuesPart}) RETURNING *`, [persoon.geheim_ind]);
}
...
export function createInsertStatements(persoon: Persoon): SqlStatement[] {
  const statements: SqlStatement[] = [createLo3PlInsertStatement(persoon), createLo3PlPersoonInsertStatement(persoon)];
