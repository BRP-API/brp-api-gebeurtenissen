...
async function getAccessToken(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to obtain token from ${tokenUrl}: ${response.status} ${response.statusText}`,
    );
  }
...
async function createClientScope(
  ...
  const response = await fetch(
    `${oauthConfig.brpApiRealm.adminBaseUrl}/client-scopes`,
    {
      method: 'POST',
      headers: {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to create client scope '${scopeName}': ${response.status} ${response.statusText}`,
    );
  }
...
export function generateClientJSON(
  clientId: string,
  clientSecret: string,
): any {
  return {
...
async function createClient(
  ...
  const response = await fetch(
    `${oauthConfig.brpApiRealm.adminBaseUrl}/clients`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to create client '${clientId}': ${response.status} ${response.statusText}`,
    );
  }
...
export function generateProtocollMapperJSON(
  oin: string,
  afnemerID: string,
  gemeenteCode?: string,
): any {
...
async function addProtocolMapperToClient(
  ...
  const response = await fetch(
    `${oauthConfig.brpApiRealm.adminBaseUrl}/clients/${clientUuid}/protocol-mappers/models`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      ...
      body: JSON.stringify(
        generateProtocollMapperJSON(oin, afnemerID, gemeenteCode),
      ),
    },
  );

  logger.debug(
    `add protocol mapper to client '${clientUuid}', oin='${oin}', afnemerID='${afnemerID}', gemeenteCode='${gemeenteCode}'`,
    {response: response},
  );

  if (!response.ok) {
    throw new Error(
      `Failed to add protocol mapper to client '${clientUuid}': ${response.status} ${response.statusText}`,
    );
  }
}

async function addOptionalClientScopeToClient(
  adminToken: string,
  clientUuid: string,
  scopeUuid: string,
): Promise<void> {
  const response = await fetch(
    `${oauthConfig.brpApiRealm.adminBaseUrl}/clients/${clientUuid}/optional-client-scopes/${scopeUuid}`,
    {
      method: 'PUT',
      headers: {
        Authorization: `Bearer ${adminToken}`,
  ...
  logger.debug(
    `add optional client scope '${scopeUuid}' to client '${clientUuid}'`,
    {response: response},
  );

  if (!response.ok) {
    throw new Error(
      `Failed to add optional client scope '${scopeUuid}' to client '${clientUuid}': ${response.status} ${response.statusText}`,
    );
  }
}
...
export async function setupClient(afnemer: Afnemer): Promise<void> {
  ...
  afnemer.idpId = await createClient(
    adminToken,
    afnemer.aanduiding,
    afnemer.clientSecret!,
  );

  await addProtocolMapperToClient(
    adminToken,
    afnemer.idpId,
    afnemer.oin!,
    afnemer.afnemerId!,
    afnemer.gemeenteCode,
  );
  await addOptionalClientScopeToClient(
    adminToken,
    afnemer.idpId,
    afnemer.idpScopeId,
  );
...
async function deleteClient(
  ...
  const response = await fetch(
    `${oauthConfig.brpApiRealm.adminBaseUrl}/clients/${clientUuid}`,
    {
      method: 'DELETE',
      headers: {
        Authorization: `Bearer ${adminToken}`,
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to delete client '${clientUuid}': ${response.status} ${response.statusText}`,
    );
  }
}

async function deleteClientScopes(
  adminToken: string,
  scopeUuid: string,
): Promise<void> {
  const response = await fetch(
    `${oauthConfig.brpApiRealm.adminBaseUrl}/client-scopes/${scopeUuid}`,
    {
      method: 'DELETE',
      headers: {
        Authorization: `Bearer ${adminToken}`,
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to delete client scope '${scopeUuid}': ${response.status} ${response.statusText}`,
    );
  }
}
...
export async function fetchClientVoorAfnemer(afnemer: Afnemer): Promise<any> {
  ...
  const response = await fetch(
    `${oauthConfig.brpApiRealm.adminBaseUrl}/clients?clientId=${afnemer.aanduiding}`,
    {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${adminToken}`,
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch client for afnemer '${afnemer.aanduiding}': ${response.status} ${response.statusText}`,
    );
  }
